 ...
 * <p>
 * Thanks to http://thompsonng.bLoggerspot.com/2010/11/rtmp-part-10-handshake.html for some very useful information on
...
//        out.write(new byte[]{(byte) 0x80, 0x00, 0x07, 0x02}); // Bytes 4 - 7: Flash player version: 11.2.202.233
        ...
        Logger.d(TAG, "writeC1(): Calculating digest offset");
        ...
        Logger.d(TAG, "writeC1(): (real value of) digestOffset: " + digestOffset);


        Logger.d(TAG, "writeC1(): recalculated digestOffset: " + absoluteDigestOffset);
        ...
        Logger.d(TAG, "writeC1(): digestOffsetBytes: " + Util.toHexString(digestOffsetBytes));  //Util.unsignedInt32ToByteArray((digestOffset % 728))));

        // Create random bytes up to the digest offset point
        byte[] partBeforeDigest = new byte[absoluteDigestOffset];
        Logger.d(TAG, "partBeforeDigest(): size: " + partBeforeDigest.length);
        random.nextBytes(partBeforeDigest);

        Logger.d(TAG, "writeC1(): Writing timestamp and Flash Player version");
        ...
        Logger.d(TAG, "partAfterDigest(): size: " + partAfterDigest.length);
        random.nextBytes(partAfterDigest);


        // Set the offset byte
//        if (digestOffset > 772) {                      
        Logger.d(TAG, "copying digest offset bytes in partBeforeDigest");
        System.arraycopy(digestOffsetBytes, 0, partBeforeDigest, 772, 4);
//        } else {
        // Implied offset of partAfterDigest is digestOffset + 32
///        Logger.d(TAG, "copying digest offset bytes in partAfterDigest");
///        Logger.d(TAG, " writing to location: " + (DIGEST_OFFSET_INDICATOR_POS - digestOffset - SHA256_DIGEST_SIZE - 8));
//        System.arraycopy(digestOffsetBytes, 0, partAfterDigest, (DIGEST_OFFSET_INDICATOR_POS - digestOffset - SHA256_DIGEST_SIZE - 8), 4);
//        }

        Logger.d(TAG, "writeC1(): Calculating digest");
        ...
        Logger.d(TAG, "writeC1(): writing C1 packet");
    ...
    public final void readS1(InputStream in) throws IOException {
        ...
        if (totalBytesRead != HANDSHAKE_SIZE) {
            ...
            Logger.d(TAG, "readS1(): S1 total bytes read OK");
        }
    }
    ...
    public final void readS2(InputStream in) throws IOException {
        ...
        if (totalBytesRead != remainingBytes) {
            ...
            Logger.d(TAG, "readS2(): S2 total bytes read OK");
        }
