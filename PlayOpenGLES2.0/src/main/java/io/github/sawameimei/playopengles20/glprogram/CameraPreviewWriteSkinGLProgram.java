...
public class CameraPreviewWriteSkinGLProgram implements TextureGLProgram {
    ...
    public CameraPreviewWriteSkinGLProgram(Context context, float[] textureM, int viewportWidth, int viewportHeight) {
        this.mContext = new WeakReference<>(context);
        this.mTextureM = textureM;
        this.mViewportWidth = viewportWidth;
        this.mViewportHeight = viewportHeight;
        mInputTextureId[0] = TextureHelper.loadOESTexture();
    }

    @Override
    public void compileAndLink() {
        mVertexShaderHandle = ShaderHelper.compileShader(GLES20.GL_VERTEX_SHADER, RawResourceReader.readTextFileFromRawResource(mContext.get(), R.raw.camera_preview_write_skin_vertex_sharder));
        mFragmentShaderHandle = ShaderHelper.compileShader(GLES20.GL_FRAGMENT_SHADER, RawResourceReader.readTextFileFromRawResource(mContext.get(), R.raw.camera_preview_write_skin_fragment_sharder));
    ...
    public void drawFrame() {
        ...
        GLES20.glBindTexture(GLES11Ext.GL_TEXTURE_EXTERNAL_OES, mInputTextureId[0]);
        ...
        GLES20.glUniform1f(muTexelWidthOffsetLoc, 2.0F / mViewportWidth);
        GLES20.glUniform1f(muTexelHeightOffsetLoc, 2.0F / mViewportHeight);
